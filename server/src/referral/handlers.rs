...
struct AccrualEntry {
    ...
    /// All attributed volume (referral-set OR builder).
    volume_quote_e8: String,
    /// Volume routed through OUR builder only — the earning volume.
    builder_volume_quote_e8: String,
    fill_count: u64,
    last_update_ms: u64,
}
...
pub async fn stats_referral_accrual_handler(
    ...
    for u in &batch.users {
        ...
        match u.from.as_deref() {
            ...
            Some(s) => {
                ...
                if from_day <= today {
                    for (_day, stats) in db.user_daily(&addr, from_day, today) {
                        ...
                        builder_volume = builder_volume.saturating_add(stats.builder_volume_quote_e8);
            ...
            None => {
                if let Some(stats) = db.get(&addr) {
                    ...
                    builder_volume = stats.builder_volume_quote_e8;
        ...
        out.push(AccrualEntry {
            ...
            builder_volume_quote_e8: builder_volume.to_string(),
            fill_count,
            last_update_ms,
        });
    }
