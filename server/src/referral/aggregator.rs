...
fn apply_batch(
    ...
    for fill in batch.events_ref() {
        ...
        for key in [cum_key(&addr).to_vec(), day_key(&addr, day).to_vec()] {
            ...
            accumulate(entry, &fill.1, block_ms, referral_hit, referral_reward_rate, builder_fee_e8);
...
fn accumulate(
    ...
    if notional.is_finite() && notional > 0.0 {
        ...
        if scaled >= 0.0 && scaled < u128::MAX as f64 {
            stats.volume_quote_e8 = stats.volume_quote_e8.saturating_add(scaled as u128);
        }
    }
