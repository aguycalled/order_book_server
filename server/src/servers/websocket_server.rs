...
pub async fn run_websocket_server(config: ServerConfig) -> Result<()> {
    ...
    let listener = { OrderBookListener::new(Some(snapshot_tx.clone()), Some(hft_tx.clone()), ignore_spot) };
...
fn ws_handler(
    ...
    tokio::spawn(async move {
        ...
        handle_socket(ws, snapshot_tx, hft_tx, listener, market_filter, bbo_only, liq_map_cache).await
    });
