...
async fn main() -> Result<()> {
    ...
    // Handle SIGTERM (supervisor/systemd stop) in addition to SIGINT (ctrl_c)
    // so the process exits intentionally rather than being killed. RocksDB
    // integrity across restarts is guaranteed by repair-on-open (see
    // `open_db_with_repair`), since detached tasks hold DB handles and a signal
    // can't run their destructors.
    let mut sigterm =
    ...
    tokio::select! {
        ...
        _ = sigterm.recv() => {
            log::info!("SIGTERM received, exiting gracefully");
        }
    }
